...
# install.packages('multicore', dependencies = TRUE)
install.packages('rbenchmark', dependencies = TRUE)
# install.packages('compiler', dependencies = TRUE) in base
#' 
...
#'   
#' 
#' This is an example of an R Markdown file written in R Notebook style, i.e. as an R script.
#' All your ouput from a series of R commands: the commands, the output and graphs are placed in a single HTML file. Optionally, output can also go into a pdf file or a doc file.
#' 
...
#' yscs and p3d.
#' 
if(!require(yscs, quietly = TRUE)) devtools::install_github('gmonette/yscs')
...
#
tab( ~ colour + sex, Arrests, pct = 2, test = TRUE)
#
# Using the name of the variable:
#
with( Arrests, tab( colour, sex, pr = 2))
# Many functions use different conventions for different arguments
#'
...
fit2i <- glm(released ~ colour * Year + age + sex +
              employed + citizen + checks, Arrests,
              family = binomial)
summary(fit2i)
...
summary(fit) # what do the coefficients and p-values tell you?
# **** WHICH COEFFICIENTS CAN YOU INTERPRET 
